    ...
    config.promises.content.then(() => {
      if (config.gameData.tradeInfo.trade_offers.length === 0) config.loadStates.content = false;
        ...
        getPromise('#tradeOfferDiv').then(() => {
          ...
          const tradePlanetData = tradeData[config.gameData.planetCoords.string];
          ...
          const $statsTable = lwmJQ(`${`${'<table id="lwm_tradeStats"><tbody>'
                        ...
                        + '</tr>'
                        + '<tr>'
                        + '<td class="">'}${config.gameData.planetCoords.string}</td>`
                        ...
                        + '<tr>'
                        + '<td class="">Total</td>'
                        + `<td class="roheisenVariable">${siteWindow.jQuery.number(savedResTotal.fe, 0, ',', '.')}</td>`
                        + `<td class="kristallVariable">${siteWindow.jQuery.number(savedResTotal.kris, 0, ',', '.')}</td>`
                        + `<td class="frubinVariable">${siteWindow.jQuery.number(savedResTotal.frub, 0, ',', '.')}</td>`
                        + `<td class="orizinVariable">${siteWindow.jQuery.number(savedResTotal.ori, 0, ',', '.')}</td>`
                        + `<td class="frurozinVariable">${siteWindow.jQuery.number(savedResTotal.fruro, 0, ',', '.')}</td>`
                        + `<td class="goldVariable">${siteWindow.jQuery.number(savedResTotal.gold, 0, ',', '.')}</td>`
                        + '</tr>'
                        + '</tbody></table>');

          lwmJQ('#tradeOfferDiv').before($statsTable);
